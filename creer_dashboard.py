...
# ==========================================
ALBUM_TRACKS = {
    "Yours Truly": ["Honeymoon Avenue", "Baby I___0", "Right There", "Tattooed Heart", "Lovin' It", "Piano", "Daydreamin'", "The Way", "You'll Never Know", "Almost Is Never Enough", "* Popular Song", "Better Left Unsaid"],
    "Yours Truly (Tenth Anniversary Edition)": ["Honeymoon Avenue", "Baby I___0", "Right There", "Tattooed Heart", "Lovin' It", "Piano", "Daydreamin'", "The Way", "You'll Never Know", "Almost Is Never Enough", "* Popular Song", "Better Left Unsaid", "Honeymoon Avenue - Live from London", "Daydreamin' - Live from London", "Baby I - Live from London", "Tattooed Heart - Live from London", "Right There - Live from London (feat. Big Sean)", "The Way - Live from London (feat. Mac Miller)"],
    ...
    "petal": ["kiss me", "hate that i made you love me", "petal", "stay", "oh well", "big feelings", "freak", "warning signs (interlude)", "like i do", "never get over me", "bad thing (bunny hop)", "nowhere, nobody"]
}
...
# 2. On filtre les chansons à +1 Milliard et on les trie
...
for b in sorted(df_billion['Billions'].unique(), reverse=True):
    ...
    for idx, row in df_tier.iterrows():
        ...
        <div class="plaque-card" style="border-color: {color};" onclick="afficherDetailsChanson('{uid_safe}')" title="Voir les graphiques de {titre_escape}">
            ...
            <div class="vinyl" style="background-image: url('{cover}'); border-color: {color};">
                <div class="vinyl-hole"></div>
            </div>
            <div class="plaque-song">{titre_escape}</div>
...
ALBUM_VISUALS = {
    ...
    "petal":{
        "source": "petal",
        "cover": "https://static.fnac-static.com/multimedia/Images/FR/NR/81/b7/35/20297601/1541-1/tsp20260513115042/petal.jpg"
    }
}
...
max_bar_height = 500 # La barre la plus haute fera 300 pixels exacts
...
for nom_album, tracks in ALBUM_TRACKS.items():
    uids = [resolve_track_id(t) for t in tracks]
...
for i in range(min(15, len(dates)-1)):
    ...
    for nom_album, df_a in album_history.items():
        if d_actuel in df_a.index and d_veille in df_a.index:
            s_act = df_a.loc[d_actuel, 'Streams_num']
            d_act = df_a.loc[d_actuel, 'Daily_num']
            s_vei = df_a.loc[d_veille, 'Streams_num']
            
            idx_alb = album_keys_list.index(nom_album)
            nom_alb_escape = html.escape(nom_album)
            link_alb = f"<a href=\"javascript:void(0)\" onclick=\"afficherDetailsAlbum({idx_alb})\" class=\"song-link\">{nom_alb_escape}</a>"
            
            # Franchissement d'un cap de 100M (Album)
            if (s_act // 100_000_000) > (s_vei // 100_000_000):
                ...
                news_items.append({
                    "date": date_disp, "type": "album_milestone", "icon": "💽",
            ...
            if d_act > 500_000:
                ...
                if not a_past.empty:
                    higher_past = a_past[a_past >= d_act]
                    if higher_past.empty:
                        news_items.append({
                            "date": date_disp, "type": "album_peak", "icon": "🚀",
                            "text": f"The album {link_alb} reached a new tracking peak of <b>{format_en(d_act)}</b> daily streams!"
                        })
                    else:
                        last_higher_date = higher_past.index.max()
                        days_since = (curr_date_obj - datetime.strptime(last_higher_date, "%Y-%m-%d")).days
                        if days_since >= 25:
                            ...
                            news_items.append({
                                "date": date_disp, "type": "album_peak", "icon": "📈",
                                "text": f"The album {link_alb} reached <b>{format_en(d_act)}</b> daily streams, its highest since {last_d}!"
                            })

    # ⚔️ DÉTECTION DES DÉPASSEMENTS (Overtakes)
    ...
    # 🎵 ANALYSE DES CHANSONS (Caps et Highest Since...)
    for uid in df_actuel['Unique_ID']:
        s_act = dict_act.get(uid, 0)
        d_act = pivot_daily.loc[d_actuel, uid] if uid in pivot_daily.columns else 0
        ...
        if d_act > 100_000:
            ...
            if not s_past.empty:
                higher_past = s_past[s_past >= d_act]
                if higher_past.empty:
                    news_items.append({
                        "date": date_disp, "type": "peak", "icon": "🚀",
                        "text": f"{link_titre} reached a new tracking peak of <b>{format_en(d_act)}</b> daily streams!"
                    })
                else:
                    last_higher_date = higher_past.index.max()
                    days_since = (curr_date_obj - datetime.strptime(last_higher_date, "%Y-%m-%d")).days
                    if days_since >= 25: 
                        ...
                        news_items.append({
                            "date": date_disp, "type": "peak", "icon": "📈",
                            "text": f"{link_titre} reached <b>{format_en(d_act)}</b> daily streams, its highest since {last_d}!"
                        })
    ...
    for date_disp, items in grouped_news.items():
        ...
        for item in items:
            ...
            if item['type'] == "milestone":
                ...
                badge_text = "Song Milestone"
            elif item['type'] == "album_milestone":
                badge_class = "badge-milestone"
                badge_text = "Album Milestone"
            elif item['type'] == "peak":
                badge_class = "badge-peak"
                badge_text = "Song Peak"
...
<head>
    ...
    <style>
        ...
        .plaque-grid {{ display: flex; gap: 30px; justify-content: center; flex-wrap: wrap; margin-bottom: 40px; }}
        .plaque-card {{ background: #111; padding: 20px; border-radius: 8px; width: 220px; border: 2px solid #444; text-align: center; box-shadow: 0 15px 30px rgba(0,0,0,0.5); position: relative; overflow: hidden; transition: transform 0.3s; cursor: pointer; }}
        .plaque-card:hover {{ transform: translateY(-10px); }}
        .plaque-title {{ font-size: 0.9em; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 20px; font-weight: bold; }}
        /* Le vinyle : l'image prend toute la place, et on ajoute une ombre interne */
        .vinyl {{ width: 140px; height: 140px; border-radius: 50%; margin: 0 auto 20px auto; border: 4px solid #333; position: relative; box-shadow: 0 0 15px rgba(0,0,0,0.8); background-size: cover; background-position: center; animation: spin 4s linear infinite; }}
        /* Le vrai trou au centre */
        .vinyl-hole {{ position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 16px; height: 16px; border-radius: 50%; background: #111; border: 2px solid #222; z-index: 2; box-shadow: inset 0 2px 5px rgba(0,0,0,0.8); }}
